...
pragma solidity ^0.8.0;
contract Weirdo is ERC721, ERC721Enumerable, Ownable, Pausable {
  ...
  uint256 public MAX_MINTABLE = 20;
  uint256 public mintPrice = 0.0001 ether;
  uint256 public maxPurchaseCount = 20;
  uint256 public totalWeirdo = 0;
  ...
  function _mintTokens(uint256 numberOfTokens) internal {
    for (uint256 i = 0; i < numberOfTokens; i++) {
      _mint(msg.sender, totalWeirdo + 1);
      totalWeirdo += 1;
    }
  }
  ...
  * we surpass MAX_MINTABLE Weirdo the mintTokens is
  ...
  */
  modifier mintCountMeetsSupply(uint256 numberOfTokens) {
    require(
      totalWeirdo + numberOfTokens <= MAX_MINTABLE,
  ...
  modifier meetsRecyclingSupply() {
    require(
      recycleSupply > 0,
      "There are no Weirdo available via recycling"
    );
    _;
  }
  ...
  modifier meetsRecyclingCost(uint256[] memory tokenIds) {
    require(
      ...
      "Incorrect number of Weirdo passed for recycling"
    );
    _;
  }
  ...
  modifier meetsOwnership(uint256[] memory tokenIds) {
    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(this.ownerOf(tokenIds[i]) == msg.sender, "You may only recycle Weirdo you own");
    }
    _;
  }
